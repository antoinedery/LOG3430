...
def evaluate(): #pragma: no cover
    ...
    for e_mail in new_emails["dataset"]:
        ...
        if ((analyzer.is_spam(subject, body, False, False, True))) and (spam == "true"):
            tp += 1
        if (not (analyzer.is_spam(subject, body, False, True, False))) and (spam == "false"):
            tn += 1
        if ((analyzer.is_spam(subject, body, True, False, True))) and (spam == "false"):
            fp += 1
        if (not (analyzer.is_spam(subject, body, True, True, False))) and (spam == "true"):
            fn += 1
        total += 1
